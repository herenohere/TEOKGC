...
class LinkGraph:
    ...
    def get_neighbor_ids(self, entity_id: str, max_to_keep=10) -> List[str]:
        ...
        # Transpose the edge list to create edge_index
...
def generate_edge_index(link_graph: LinkGraph, entity_dict: EntityDict):
        edge_list = []

        for head_id, tail_ids in link_graph.graph.items():
            head_idx = entity_dict.entity_to_idx(head_id)
            for tail_id in tail_ids:
                tail_idx = entity_dict.entity_to_idx(tail_id)
